...
/* URl detection Regex Function*/

function urlify(text) {
    //var urlRegex = /(https?:\/\/[^\s]+)/g;
	var urlRegex = /(\b(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/ig;
    return text.replace(urlRegex, function(url) {
        return '<span><a href="' + url + '">' + 'https://stu.po/me' + '</a></span>';
    });
}

$("#divPost p").each(function(){
	...
	$('.postDescription').each(function(event) { /* select all divs with the item class */
		...
		/*detecting the url if any?*/
            ...
            if ($(this).html().length > max_length) { /* check for content length */
                ...
                $(this).html(short_content+
                '<span class="read_more"><b>View</b></span>' +
                    '<span class="more_text" style="display:none;">' + long_content + '</span>' );
                /* Alter the html to allow the read more functionality */

                $(this).find('span.read_more').click(function (event) { /* find the a.read_more element within the new html and bind the following code to it */
                ...
                if(window.innerWidth > 768){
                    var short_content = $(this).html().substr(0, max_length);
                    $(this).html(short_content + "<b>View</b>");
                }
            }
